...
const novaPostagem = async (req, res) =>{
    ...
    if(!fotos || fotos.length === 0){
        return res.status(404).json('E preciso informar ao menos uma foto');
    }
    ...
    try {
        ...
        if(!postagem){
            return res.status(400).json('Nao foi possivel concluir a postagem');
        }
...
const curtir = async (req, res) => {
    ...
    try {
        ...
        if(!postagem) {
            return res.status(404).json('Postagem nao encontrada');
        }
        ...
        if(!curtida){
            return res.status(404).json('Nao foi possivel curtir essa postagem.');
        }
        return res.status(404).json('Postagem curtida com sucesso');
    } catch (error) {
        return res.status(404).json(error.message);
    }
}
...
const comentar = async (req, res) => {
    ...
    if(!texto){
        return res.status(404).json('Para comentar nessa postagem e necessario informar o texto');
    }
    ...
    try {
        ...
        if(!comentario){
            return res.status(404).json('Nao foi possivel comentar essa postagem.');
        }

        return res.status(404).json('Postagem comentada com sucesso');
    } catch (error) {
        return res.status(404).json(error.message);
    }
}
...
const feed = async (req, res) => {
    ...
    try {
        //const postagens = knex('postagens').limit(10).offset(0);
