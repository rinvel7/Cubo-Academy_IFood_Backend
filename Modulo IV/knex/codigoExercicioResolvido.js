...
app.get('/', async (req, res)=> {
    ...
    //const quantidadeMedicamento = await knex('farmacia').count()
   ...
   //const SomaCategoria = await knex('farmacia').select('categoria').sum('estoque').whereNotNull('categoria').groupBy('categoria')
    ...
    const quantidadeCategoria = await knex('farmacia').select('categoria').count().whereNotNull('categoria').groupBy('categoria')
    ...
    const quantidadeUsuario = await knex('usuarios').select('idade').count().where('idade', '>=', 18).groupBy('idade').debug();
